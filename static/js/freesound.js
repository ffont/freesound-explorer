(function () {
    ...
    var search = function(options, uri, success, error){
        if(options.analysis_file){
                makeRequest(makeUri(uri), success,error,null,
                            SoundCollection, 'POST',makeFD(options));
        }
        else{
                makeRequest(makeUri(uri), success,error,options,
                            SoundCollection);
        }
    };
    ...
    var SoundObject = function (jsonObject){ 
        jsonObject.getAnalysis = function(filter, success, error, showAll){
            makeRequest(makeUri(uris.soundAnalysis,[jsonObject.id,filter?filter:""]),
                        success,error);
        };
        
        jsonObject.getSimilar = function (success, error, params){
            makeRequest(makeUri(uris.similarSounds,[jsonObject.id]),
                        success,error, params,SoundCollection);
        };

        jsonObject.getComments = function (success, error){
            makeRequest(makeUri(uris.comments,[jsonObject.id]),
    ...
    return {
            ...
            textSearch: function(query, options, success, error){
                ...
                search(options,uris.textSearch,success,error);
            },
            ...
            getSound: function(soundId,success, error){
                makeRequest(makeUri(uris.sound, [soundId]),
                            success,error,{}, SoundObject);
            },
            ...
            getLoginURL: function(){
                ...
                var login_url = makeUri(uris.logoutAuthorize);
...
(function () {
    // Set API tokens
    freesound.setClientSecrets("CLIENT_ID", "CLIENT_SECRET");
    freesound.setToken("CLIENT_SECRET");
})();
